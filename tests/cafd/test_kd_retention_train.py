...
import cafd.kd_retention_train as runner
from cafd.kd_retention_objective import route_reward_groups, teacher_coefficients
...
def setup(monkeypatch):
    ...
    cfg.update(kd_retention=False, token_chunk=2, rollouts_per_prompt=2, grad_clip=1., control_rollouts=2)
...
def test_smoke_profile_is_small_and_explicitly_not_formal():
    ...
    assert smoke["total_rounds"] == 2 and smoke["max_new_tokens"] == 64
...
def test_retention_four_groups_dense_accumulation_one_step(setup,monkeypatch):
    student, behavior, reference, pool, optimizer, cfg = setup
    cfg.update(kd_retention=True,rollouts_per_prompt=8)
    rows = [
        make_row("d", [0., 0.], [[3, 4], [4, 2]]),
        make_row("r", [.1, .4], [[5], [6, 7, 8]]),
    ...
    z = sum(len(record["completion_ids"]) for group in groups for record in group)
    assert z == 68
    reward=torch.tensor([row["rewards"]*4 for row in rows])
    routing = route_reward_groups(reward,full_pass=reward==1.,kd_retention=True)
    coefficients = teacher_coefficients(1.25, ["r0", "r1", "r2"])
    expected_loss = 0.
    for j, group in enumerate(groups):
        if routing.skip[j]:
        ...
        for l, record in enumerate(group):
            ...
            if routing.rl[j]:
                ...
                ratio = (selected - old).exp()
                advantage = routing.advantages[j, l]
                loss = loss -torch.minimum(ratio * advantage, ratio.clamp(.8, 1.2) * advantage).sum() / z
            expected_loss = expected_loss + loss
    expected_loss.backward()
    torch.nn.utils.clip_grad_norm_(expected.parameters(), cfg["grad_clip"])
    expected_optimizer.step()
    ...
    def step(*args,**kwargs):
        steps.append(1);return original_step(*args,**kwargs)
    def clip(*args,**kwargs):
        clips.append(1);return original_clip(*args,**kwargs)
    ...
    assert (log["kd_only_groups"],log["rl_only_groups"],log["kd_rl_groups"],log["skip_groups"])==(1,1,1,1)
    assert (log["kd_tokens"],log["rl_tokens"],log["overlap_tokens"])==(32,32,16)
    assert log["distill_groups"]==2 and log["rl_groups"]==2
    assert len(steps)==len(clips)==1
    assert log["optimizer_step"] and log["kd_loss"]>0
    assert costs["training_reference_scored_tokens"]==32
    assert costs["training_teacher_scored_tokens"]==96
    assert len(generated)==4 and all(len(g)==8 for g in generated)
    for actual,desired in zip(student.parameters(),expected.parameters()):
        torch.testing.assert_close(actual,desired,atol=2e-6,rtol=2e-6)
    for model,snapshot in zip([reference,*pool.models.values()],frozen_states):
        assert_state_same(model,snapshot)
...
def test_four_masks_require_verifier_and_strict_spread():
    rewards=torch.tensor([[0.]*8,[.1,.2]*4,[.1,1.]*4,[1.]*8],dtype=torch.float64)
    old=route_reward_groups(rewards,full_pass=rewards==1.)
    new=route_reward_groups(rewards,full_pass=rewards==1.,kd_retention=True)
    assert old.distill.tolist()==[True,False,False,False]
    assert new.distill.tolist()==[True,True,False,False]
    assert old.rl.tolist()==new.rl.tolist()==[False,True,True,False]
    assert new.skip.tolist()==[False,False,False,True]
    with pytest.raises(ValueError,match="explicit"):
        route_reward_groups(rewards,kd_retention=True)
    near=torch.tensor([[.1,.1+1e-14]],dtype=torch.float64)
    assert route_reward_groups(near,full_pass=near==1.,kd_retention=True).rl.item()

def test_replay_all_boundaries_and_resume():
    from cafd.kd_retention_replay import ReplayController,U_BY_BLOCK
    from cafd.mpc_controller import MPCConfig
    config=MPCConfig(route_intervals=5,total_rounds=200,family_weights=(1.,),seed=2027)
    c=ReplayController(config,[[.1,0.,.5]])
    for i,u in enumerate(U_BY_BLOCK):
        decision=c.choose_action()
        assert decision["u"]==u and decision["completed_rounds_before"]==10*i
        assert decision==c.choose_action()
        assert "mpc_recommendation" in decision
        c.observe([[.1,0.,.5]])
        c=ReplayController.from_state_dict(c.state_dict())
        assert c.u==u
    assert c.done and c.completed_rounds==200
