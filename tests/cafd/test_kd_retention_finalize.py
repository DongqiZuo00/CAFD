...
def _clarified_retention_fixture(fixture, *, actual=199, approved=True):
    root, old_run, rows = fixture
    run_id = "mistral_cafd_kd_retention_replay_s2027"
    run = old_run.with_name(run_id)
    old_run.rename(run)
    selected = json.loads((run / "selected.json").read_text())
    selected["checkpoint"] = str(run / "round40")
    write_json(run / "selected.json", selected)
    complete = json.loads((run / "complete.json").read_text())
    complete["actual_updates"] = actual
    complete["selected"] = {k:v for k,v in selected.items() if k not in {"status","selection_rule"}}
    if approved:
        complete["protocol_clarification"] = {
...
def test_user_approved_skip_preserves_true_update_count_and_identity(fixture):
    root,run,run_id = _clarified_retention_fixture(fixture)
    context=final.validate_finalization(root,run_id)
    assert context["identity"]["actual_optimizer_updates"] == 199
    assert context["identity"]["protocol_clarification"]["skipped_all_success_rounds"] == [194]
    assert context["identity"]["training_rounds"] == 200

@pytest.mark.parametrize("actual,approved",[(199,False),(198,True)])
def test_unapproved_or_different_shortfall_still_rejected(fixture,actual,approved):
    root,run,run_id = _clarified_retention_fixture(fixture,actual=actual,approved=approved)
    with pytest.raises(RuntimeError,match="explicit user-approved"):
        final.validate_finalization(root,run_id)

def test_changed_clarification_is_rejected(fixture):
    root,run,run_id = _clarified_retention_fixture(fixture)
    complete=json.loads((run/"complete.json").read_text())
    complete["protocol_clarification"]["skipped_all_success_rounds"]=[193]
    write_json(run/"complete.json",complete)
    with pytest.raises(RuntimeError,match="explicit user-approved"):
        final.validate_finalization(root,run_id)
