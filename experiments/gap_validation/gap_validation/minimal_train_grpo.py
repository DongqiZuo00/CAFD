...
def math_reward(completions, solution, **kwargs):
    ...
    return [float(verify_math(text, gold).correct) for text, gold in zip(texts, solution)]
...
def dataset_for_math(path: Path) -> Dataset:
    ...
    for row in load_jsonl(path):
        rows.append(
            {"prompt": pair_messages("math", row["prompt"]), "solution": row["solution"], "id": row["id"]}
        )
...
def main() -> None:
    ...
    if 32 % world_size:
        raise ValueError("world size must divide 32 rollout slots per update")
    # TRL's train batch is measured in generated sequences after RepeatSampler expands each
    # prompt num_generations times. A global batch of 32 therefore represents exactly
    # eight unique prompts with four responses each in every optimizer update.
    rollout_slots_per_device = 32 // world_size
    config = GRPOConfig(
        ...
        per_device_train_batch_size=rollout_slots_per_device,
    ...
    if trainer.is_world_process_zero():
        ...
        summary = {
            ...
            "verifier_calls": int(trainer.state.global_step) * 32,
