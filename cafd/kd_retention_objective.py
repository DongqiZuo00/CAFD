...
def route_reward_groups(
    ...
    *,
    ...
    rl = spread > 0.0
    if kd_retention and full_pass is None:
        raise ValueError("KD retention requires explicit full verifier flags")
    all_fail = ~(expected_full_pass if full_pass is None else full_pass.bool()).any(dim=-1)
    distill = all_fail if kd_retention else ((spread == 0.0) & all_fail)
    skip = ~(rl | distill)
...
def mixed_objective_reference(
    ...
    *,
    ...
    routing = route_reward_groups(rewards, full_pass, advantage_epsilon=advantage_epsilon, kd_retention=kd_retention)
