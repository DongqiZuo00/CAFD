...
def validate_finalization(root: Path, run_id: str) -> dict[str, Any]:
    ...
    root = root.resolve()
    if not re.fullmatch(r"mistral_cafd_(?:mpc|kd_retention)_[a-zA-Z0-9_]+", run_id) or run_id.endswith("_smoke"):
    ...
    clarification = complete.get("protocol_clarification")
    if "kd_retention" in run_id and actual != 200:
        approved = {
        ...
        if actual != 199 or clarification != approved:
            raise RuntimeError("KD retention requires200 updates or the explicit user-approved199-update protocol")
    ...
    if clarification is not None:
        identity["protocol_clarification"] = clarification
