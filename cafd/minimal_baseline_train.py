"""Isolated GRPO and absolute-target KD+RL with the fixed CAFD protocol.
...
from .minimal_baseline_objective import (teacher_coefficients, route_reward_groups, route_training_groups,
                            absolute_teacher_coefficients, CONDITIONS, exact_linear_forward_kl,
                            exact_linear_token_log_probs, clipped_rl_loss)
from .minimal_baseline_runtime import (load_hidden, independent_copy, refresh_behavior, TeacherPool,
                         absolute_frozen_sources,
...
def configuration(profile, condition):
    ...
    cfg.update(condition=condition,
        method="Pure GRPO + old schedule diagnostic replay" if condition=="grpo"
               else "Absolute-target retention KD+RL + old schedule replay",
        target="none" if condition=="grpo" else "(1-alpha)*Tm+alpha*Tnext",
        kd_retention=condition=="absolute_kd", lambda_kd=0. if condition=="grpo" else 1.,
        budget_definition="fixed_rollout_rounds_actual_optimizer_updates_may_be_lower",
        diagnostic_teacher_overhead=True,
        diagnostic_target="S0+(1-alpha)*Tm+alpha*Tnext-T0",
        diagnostic_only_control=True,
        comparison_run="mistral_cafd_kd_retention_replay_s2027")
    return cfg
...
def count_sources(costs, records, coefficients, purpose, *, include_reference=True):
    ...
    add(costs, purpose+"_reference_scored_tokens", tokens if include_reference else 0)
    add(costs, purpose+"_teacher_prompt_tokens", prompts*len(coefficients))
    add(costs, purpose+"_reference_prompt_tokens", prompts if include_reference else 0)
...
def train_round(student,behavior,reference,pool,route,optimizer,tokenizer,rows,cfg,round_index,u,costs,device):
    ...
    routing = route_training_groups(reward,verified,condition=cfg["condition"],
                                    advantage_epsilon=cfg["advantage_epsilon"])
    ...
    log.update(condition=cfg["condition"],target=cfg["target"],
        eligible_tokens=sum(n for n,d,r in zip(lengths,kd,rl) if d or r),
        skip_reason=None)
    if not z or not routing.should_step or not log["eligible_tokens"]:
        log["skip_reason"]="no_valid_tokens" if not z else "no_eligible_loss"
        return log,groups
    coefficients = absolute_teacher_coefficients(u,route) if bool(routing.distill.any()) else {}
    ...
    for j,group in enumerate(groups):
        ...
        for l,record in enumerate(group):
            ...
            scoring_started=time.monotonic()
            sources = absolute_frozen_sources(record,tokenizer,teachers,coefficients,device) if bool(routing.distill[j]) else None
            ...
            if sources is not None:
                ...
                count_sources(costs,[record],coefficients,"training",include_reference=False)
...
def validate_training_budget(training_rows,cfg,actual_updates):
    expected_rounds=list(range(1,cfg["total_rounds"]+1))
    if [row["round"] for row in training_rows] != expected_rounds:
        raise RuntimeError("training journal must contain every budgeted round exactly once")
    skipped=[]
    for row in training_rows:
        groups=row["group_routing"]
        if row.get("condition")!=cfg["condition"] or len(groups)!=cfg["prompts_per_round"]:
            raise RuntimeError("training condition or prompt count mismatch")
        total_tokens=eligible_tokens=0
        for group in groups:
            rewards,passed=group["rewards"],group["full_pass"]
            if len(rewards)!=cfg["rollouts_per_prompt"] or len(passed)!=len(rewards):
                raise RuntimeError("rollout count mismatch")
            if any(bool(p)!=(r==1.) for r,p in zip(rewards,passed)):
                raise RuntimeError("reward/verifier identity mismatch")
            expected_kd=cfg["condition"]=="absolute_kd" and not any(passed)
            expected_rl=max(rewards)>min(rewards)
            if group["use_kd"]!=expected_kd or group["use_rl"]!=expected_rl:
                raise RuntimeError("training loss eligibility mismatch")
            lengths=group["completion_tokens"]
            if len(lengths)!=len(rewards) or any(n<0 for n in lengths) or sum(lengths)!=group["valid_tokens"]:
                raise RuntimeError("completion token accounting mismatch")
            total_tokens+=group["valid_tokens"]
            if expected_kd or expected_rl:
                eligible_tokens+=group["valid_tokens"]
        if total_tokens!=row["normalization_tokens"] or eligible_tokens!=row["eligible_tokens"]:
            raise RuntimeError("global token accounting mismatch")
        if not isinstance(row["optimizer_step"],bool) or row["optimizer_step"]!=(eligible_tokens>0):
            raise RuntimeError("optimizer step does not match eligible loss")
        if not row["optimizer_step"]:
            if row["skip_reason"] not in ("no_eligible_loss","no_valid_tokens"):
                raise RuntimeError("missing legal no-update explanation")
            skipped.append(row["round"])
    observed=sum(row["optimizer_step"] for row in training_rows)
    if actual_updates!=observed:
        raise RuntimeError("actual optimizer update counter differs from journal")
    return dict(status="passed",condition=cfg["condition"],rounds=len(training_rows),
        actual_updates=observed,skipped_rounds=skipped,
        budget_definition=cfg["budget_definition"],
        prompt_presentations=len(training_rows)*cfg["prompts_per_round"],
        generated_answers=len(training_rows)*cfg["prompts_per_round"]*cfg["rollouts_per_prompt"])
...
def main():
    ...
    expected_run_id="mistral_cafd_minimal_"+args.condition+"_s2027"+("_smoke" if args.profile=="smoke" else "")
    args.run_id=args.run_id or expected_run_id
    ...
    if args.run_id != expected_run_id:
        raise ValueError("minimal baseline run ID must match condition and profile")
    ...
    if any(count > 1 for count in allocation["slurm_gpu_count_metadata"].values()):
        raise RuntimeError("minimal baseline reserves exactly one GPU")
    cfg=configuration(args.profile,args.condition)
    ...
    while not controller.done:
        ...
        if args.stop_after_round is not None and controller.completed_rounds>=args.stop_after_round and not controller.done:
            ...
            return
    if controller.completed_rounds != cfg["total_rounds"]:
        raise RuntimeError("incomplete rollout-round budget")
    training_rows=[json.loads(line) for line in (artifacts/"training_curve.jsonl").read_text().splitlines() if line.strip()]
    budget_audit=validate_training_budget(training_rows,cfg,actual_updates)
    write_json(out/"training_budget_audit.json",budget_audit)
    if best is not None:
    ...
    result=dict(status="SMOKE_PASSED" if args.profile=="smoke" else "STUDENT_TRAINING_COMPLETE",
        formal_result=args.profile=="formal",condition=cfg["condition"],
        rounds=controller.completed_rounds,actual_updates=actual_updates,budget_audit=budget_audit,
