...
from .mpc_objective import (teacher_coefficients, route_reward_groups, exact_linear_forward_kl,
                            exact_linear_token_log_probs, clipped_rl_loss)
from .mpc_runtime import (load_hidden, independent_copy, refresh_behavior, TeacherPool,
...
def train_round(student,behavior,reference,pool,route,optimizer,tokenizer,rows,cfg,round_index,u,costs,device):
    ...
    routing = route_reward_groups(reward,full_pass=verified)
    ...
    for j,group in enumerate(groups):
        ...
        for l,record in enumerate(group):
            ...
            if sources is not None:
                ...
                loss = clipped_rl_loss(current,old,routing.advantages[j,l],torch.ones_like(current,dtype=torch.bool),
                    normalization_tokens=z,clip=cfg["ratio_clip"],lambda_rl=cfg["lambda_rl"])
                log["rl_loss"] += float(loss.detach())
...
def development(student,tokenizer,rows,cfg,step,costs):
    ...
    return dict(round=step,correct=correct,total=len(records),accuracy=correct/len(records)),records
...
def main():
    ...
    args.run_id=args.run_id or ("mistral_cafd_mpc_v1_"+args.profile)
    ...
    if not re.fullmatch(r"mistral_cafd_mpc_[a-zA-Z0-9_]+",args.run_id):
    ...
    if (out/"complete.json").exists():
        raise RuntimeError("completed run: refusing overwrite")
    ...
    if args.profile=="formal" and not args.resume:
