"""Minimal baseline endpoint evaluation; S0 is predeclared and trained models are development-frozen.
...
after the fixed-round run and frozen selection are verified, or an explicit evaluation-only S0 plan.
"""
...
S0_RUN = "mistral_cafd_minimal_s0_s2027"
TRAIN_RUNS = {
    "mistral_cafd_minimal_grpo_s2027": "grpo",
    "mistral_cafd_minimal_absolute_kd_s2027": "absolute_kd",
}
...
def validate_training_budget(run, artifact, cfg, complete):
    condition = TRAIN_RUNS.get(run.name)
    if cfg.get("condition") != condition:
        raise RuntimeError("run identity and training condition differ")
    curve = _read_jsonl(artifact / "training_curve.jsonl")
    if [r.get("round") for r in curve] != list(range(1,201)):
        raise RuntimeError("exactly200 unique sampled rounds are required")
    if any(type(r.get("optimizer_step")) is not bool for r in curve):
        raise RuntimeError("invalid optimizer step flag")
    if sum(r["optimizer_step"] for r in curve) != complete["actual_updates"]:
        raise RuntimeError("actual update count differs from saved training curve")
    for record in curve:
        groups=record.get("group_routing",[])
        if len(groups)!=4:
            raise RuntimeError("four groups per round are required")
        total=eligible=0
        for group in groups:
            rewards=group.get("rewards",[]);flags=group.get("full_pass",[])
            lengths=group.get("completion_tokens",[])
            if len(rewards)!=8 or len(flags)!=8 or len(lengths)!=8:
                raise RuntimeError("eight answers per group are required")
            if any(type(flag) is not bool for flag in flags):
                raise RuntimeError("full verifier flags must be booleans")
            if any(type(n) is not int or not 0 <= n <= 2048 for n in lengths):
                raise RuntimeError("completion token lengths must be integers within the generation budget")
            if any(type(value) not in (int,float) or not math.isfinite(value) or not 0 <= value <= 1 for value in rewards):
                raise RuntimeError("training rewards must be finite values within [0,1]")
            if any(flag != (value == 1) for flag,value in zip(flags,rewards)):
                raise RuntimeError("full verifier flags disagree with rewards")
            varied=max(rewards)>min(rewards)
            kd=condition=="absolute_kd" and not any(flags)
            if group.get("use_kd") is not kd or group.get("use_rl") is not varied:
                raise RuntimeError("condition routing differs from saved verifier results")
            tokens=sum(lengths)
            if tokens != group.get("valid_tokens"):
                raise RuntimeError("completion token totals disagree")
            total+=tokens
            if kd or varied:eligible+=tokens
        if total!=record.get("normalization_tokens"):
            raise RuntimeError("global token denominator disagrees")
        if record["optimizer_step"] is not (eligible>0):
            raise RuntimeError("optimizer step does not match eligible completion tokens")
    return curve

def _s0_config():
    return dict(protocol="CAFD-MPC-2026-09-06",profile="evaluation_only",condition="s0",
        method="Original S0 (no training)",seed=2027,total_rounds=0,
        ...
        maximum_gpus=1,reserved_memory_gib=192)

def _s0_identity(root,run_id,manifest,cfg):
    if run_id!=S0_RUN or cfg!=_s0_config():
        raise RuntimeError("S0 requires its exact evaluation-only identity")
    if manifest.get("version")!="cafd_mpc_v1" or manifest.get("status")!="frozen":
        raise RuntimeError("frozen MPC manifest required")
    ...
    if test.get("count")!=132 or test.get("ids_metadata_path")!=str(IDS_REL):
        raise RuntimeError("wrong official S0 test identity")
    if test.get("source_file_metadata")!=_file_metadata(root,source):
        raise RuntimeError("S0 test source metadata changed")
    ids_meta=_read(_inside(root,IDS_REL));ids=ids_meta.get("ids")
    if ids_meta.get("source")!=str(TEST_REL) or ids_meta.get("count")!=132 or not isinstance(ids,list) or len(ids)!=132 or len(set(ids))!=132:
        raise RuntimeError("invalid S0 test IDs")
    if any(not isinstance(value,str) or not value for value in ids):
        raise RuntimeError("invalid S0 ID value")
    for split in ("optimization","control","development"):
        if set(manifest["partitions"][split]["ids"]) & set(ids):
            raise RuntimeError("S0 test IDs overlap training-side partition")
    checkpoint=_inside(root,S0_REL)
    return dict(version="cafd_minimal_final_v1",run_id=run_id,protocol=cfg["protocol"],
        checkpoint=str(checkpoint),checkpoint_metadata=checkpoint_metadata(root,checkpoint),
        selection=dict(status="frozen",selection_rule="predeclared_original_S0",round=0,checkpoint=str(checkpoint)),
        training_rounds=0,actual_optimizer_updates=0,seed=2027,
        generation=dict(max_new_tokens=2048,max_prompt_tokens=4096,sample=False,num_return_sequences=1,
            renderer="ministral3_instruct_fixed_system",stop=cfg["output_stop"],verifier=cfg["reward"]),
        test_ids=ids,test_source_metadata=test["source_file_metadata"],test_historically_evaluated=True)

def prepare_s0(root):
    root=root.resolve()
    run=_inside(root,Path("runs/cafd/experiments")/S0_RUN)
    manifest=_read(_inside(root,Path("data/cafd/mpc_v1/manifest.json")))
    cfg=_s0_config();identity=_s0_identity(root,S0_RUN,manifest,cfg)
    _publish_once(run/"config.json",_pretty(cfg))
    _publish_once(run/"data_manifest.json",_pretty(manifest))
    _publish_once(run/"evaluation_plan.json",_pretty(identity))
    return validate_s0_finalization(root,S0_RUN)

def validate_s0_finalization(root,run_id):
    root=root.resolve();run=_inside(root,Path("runs/cafd/experiments")/run_id)
    artifact=_inside(root,Path("artifacts/cafd/experiments")/run_id)
    cfg=_read(run/"config.json");manifest=_read(run/"data_manifest.json")
    identity=_s0_identity(root,run_id,manifest,cfg)
    if _read(run/"evaluation_plan.json")!=identity:
        raise RuntimeError("predeclared S0 checkpoint/test identity changed")
    return dict(root=str(root),run=str(run),artifact=str(artifact),
        test_source=str(_inside(root,TEST_REL)),identity=identity,
        training_costs=dict(gpu_hours=0.,training_generated_tokens=0,training_teacher_scored_tokens=0),
        training_job_id=None,limitations=manifest.get("limitations",[]),config=cfg)
...
def validate_finalization(root: Path, run_id: str) -> dict[str, Any]:
    ...
    root = root.resolve()
    if run_id == S0_RUN:
        return validate_s0_finalization(root,run_id)
    if run_id not in TRAIN_RUNS:
        raise RuntimeError("declared formal minimal-baseline run identity required")
    ...
    if (cfg.get("profile") != "formal" or cfg.get("protocol") != "CAFD-MPC-2026-09-06" or
            ...
            cfg.get("output_stop") != "external_closing_fence_or_generated_EOS_no_synthetic_EOS" or
            cfg.get("reward") != "hierarchical_0_.05_.10_.10+.90*pass_rate_1" or
            cfg.get("milestones") != [0, 10, 40, 80, 120, 160, 200]):
        raise RuntimeError("formal evaluation configuration changed")
    validate_training_budget(run,artifact,cfg,complete)
    ...
    identity = {
        "version": "cafd_minimal_final_v1", "run_id": run_id, "protocol": cfg["protocol"],
...
def aggregate(context, records, costs) -> dict[str, Any]:
    ...
    row = {
        "condition": context["config"].get("method","CAFD-MPC"), "benchmark": "DELTA Manufactoria-HAS",
        "backbone": "mistralai/Ministral-3-3B-Instruct-2512-BF16",
        "seed": 2027, "correct": correct, "total": 132, "accuracy": correct / 132,
        "training_rounds": identity["training_rounds"], "actual_optimizer_updates": identity["actual_optimizer_updates"],
...
def seal_result(context, records, costs) -> dict[str, Any]:
    ...
    row = summary["scores"]
    _publish_once(artifact / "final_scores.csv", _csv([row], list(aggregate(context, records, summary["evaluation_costs"])["scores"])))
...
def main():
    ...
    parser.add_argument("--prepare-s0",action="store_true")
    args = parser.parse_args()
    if Path.cwd().resolve() != ROOT:
        raise RuntimeError("all work must remain in /blue/du.j/jinjiaguo/CAFD")
    if args.prepare_s0:
        if args.run_id != S0_RUN:
            raise RuntimeError("--prepare-s0 only applies to the declared S0 baseline")
        context=prepare_s0(ROOT)
        print(json.dumps(dict(status="S0_EVALUATION_PREDECLARED",checkpoint=context["identity"]["checkpoint"],training_rounds=0,actual_updates=0)),flush=True)
        return
    ...
    with (artifact / "final_test.lock").open("a") as lock:
        ...
        if len(records) == 132:
            ...
            temporary = artifact / f".final_test_progress-{os.getpid()}-{time.time_ns()}.json"
            _publish_once(temporary, _pretty({
